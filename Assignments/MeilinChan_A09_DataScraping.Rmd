...
year1019 <- seq(2010,2019)
ashe.pswid <- '01-11-010'

ashe.10.19 <- lapply(X=year1019, FUN = scrape.NCDEQ, pswid_num = ashe.pswid) %>% bind_rows()


ggplot(ashe.10.19, aes(x=Date, y =Avg_Max_Withdrawals))+
  geom_line()+
  geom_smooth(method="loess",se=FALSE) +
  labs(title = "Asheville Water Withdrawals from 2010-2019",
       y="Avg. Max. Withdrawals (mgd)",
       x="Date")
