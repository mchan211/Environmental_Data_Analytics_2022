...
NTL.three.leg <- plot_grid(NTL.three, NTL.temp,nrow = 2, rel_heights = c(3,0.3))
...
needles.dry.violin <- ggplot(subset(NN_Litter, functionalGroup == "Needles")) +
  geom_violin(aes(x = collectDate, y = dryMass, 
                 color = nlcdClass)) +
...
facet.needles.dry <- ggplot(subset(NN_Litter, functionalGroup == "Needles")) +
  geom_violin(aes(x = collectDate, y = dryMass)) +
  xlab("Collection Year") +
  ylab("Needles Dry Mass") +
  facet_grid(~ nlcdClass)
  
print(facet.needles.dry)
...
time period was below 10 units of dry mass. When we observe Plot 6, the violin
plot does not distribute well with color set as the NLCD Classes. We cannot 
accurately see the spread of the amount of needle dry mass collected over the
period of the datatable. 
