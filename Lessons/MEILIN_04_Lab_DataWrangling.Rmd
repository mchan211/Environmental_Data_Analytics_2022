...
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #using %in% is just like using the == 
...
NTL.phys.data.90s.Tues <- NTL.phys.data %>%
...
NTL.phys.data.processed <- 
  ...
  select(lakename, sampledate:temperature_C) %>% #remember - select() is for columns, filter() is for rows
  mutate(temperature_F = (temperature_C*9/5) + 32)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 through 1999 only for July 

NTL.phys.data.90s.TuesinJuly <- NTL.phys.data.90s.Tues %>%
...
NTL.phys.data.90s.TuesinJuly.surf <- NTL.phys.data.90s.TuesinJuly %>%
  filter(depth ==0) %>%
  filter(!is.na(temperature_C)) %>% #using ! pulls all observations that are NOT NA
  summarise(mean = mean(temperature_C)) #this is adding a new column
...
NTL.phys.data.ex5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, values_to = "irradianceAll")

#Exercise 6: spread temperatureC into more than one column based on the depth
class(NTL.phys.data$depth)
+NTL.phys.data.ex6 <- pivot_wider(NTL.phys.data, values_from = temperature_C)
