...
```{r, message = FALSE}
getwd()
setwd("C:/Users/meili/OneDrive - Duke University/EDA/Environmental_Data_Analytics_2022")
library(tidyverse)
library(lubridate)
#install.packages("trend") #run stationary tasks
library(trend)
#install.packages("zoo") #replace missing observations in time series
library(zoo)
#install.packages("Kendall") #specific to kendall stationary test
library(Kendall)
#install.packages("tseries") #has functions for tseries analysis
library(tseries)
...
EnoDischarge <- read.csv(file = "Data/Processed/USGS_Site02085000_Flow_Processed.csv",stringsAsFactors = TRUE)
  
#read.csv("./Data/Processed/USGS_Site02085000_Flow_Processed.csv", stringsAsFactors = TRUE)

EnoDischarge$datetime <- as.Date(EnoDischarge$datetime, format = "%Y-%m-%d")

#Read wind speed data
wind_data <- read.csv(file="./Data/Raw/Wind_Speed_PortArthurTX.csv",
                      header=TRUE,
                      stringsAsFactors = TRUE)
wind_data$DATE <- ym(wind_data$DATE) #fixing the date through the lubridate pakcage
...
```{r}
#first step in time series analysis is to plot the variable over time just to understand what your data shows

ggplot(EnoDischarge, aes(x = datetime, y = discharge.mean)) +
  geom_line() +
  labs(x = "", y = expression("Discharge (ft"^3*"/s)")) #there is a gap - missing data from 1971-1985 --> this is an issue for time series analysis (not continuous data)

#must find a way to work around missing data - can split into two separate data sets
```
...
EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01")) #observations before 1985

EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01")) #observations after 1985
```
...
>nonmonotonic - eliminate by differencing the time series
...
>part of time series that cannot be explained by a trend or seasonality - hardest part to predict in series
...
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365) #changing variable within a dataframe into a time series; must define the starting point (1st observation - year, month) and frequency

#ts(variable, start = c(year,month), frequencey = daily/monthly/yearly) - using 365 for frequency tells R that time series is a daily series
```
...
```{r}
#DECOMPOSE THE TIME SERIES - find the trend, seasonal, and random components
?stl
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

#stl(univariate time series, seasonal window - perioidic or the the span/lags of window)

#MUST RUN DECOMPOSITION FOR EACH VARIABLE SEPARATELY FOR YOUR TIME SERIES - one by one
#using s.window = periodic will use the time period specified in the ts() function

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed) #not a monotonic trend, many instances of increase and decrease

# We can extract the components and turn them into data frames
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3]) #time series has seasonal, trend, and remainder - want to create a data frame from these columns

EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
        Observed = EnoDischarge.late$discharge.mean,     
        Date = EnoDischarge.late$datetime) #adding two more columns to the dataframe - taking info from og data frame through an observed column w/ discharge mean and adding in a date column
...
#geom_hline --> call attention to how seasonal trend components will have trend areas less than 0 HOWEVER usually enviro observations should not be less than 0
...
## Trend analysis
>want to see if there will be trends that will be repeated in future variables
...
>before analysis, want to identify any gaps in your data set - can be a challenge for this analysis
...
>if gap is not that large, can use the method below
...
```{r}
head(wind_data)
summary(wind_data$AWND) #see that there is NAs - missing observations

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #adding new column new AWND.clean using zoo package - na.approx() used to interpolate missing observations using linear or spline interpolation

#some functions have same name across packages - use the package that the function originates and then name of function - zoo::na.approx()


#if there are NAs in the MIDDLE of a dataset, DO NOT use na.omit or dropna--> MUST USE na.approx()
...
ggplot(wind_data_clean ) +
  ...
  ylab("Average Wind Speed") #can see that we interpolated the trend by connecting the line in area where the NA observations were
```
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall) >to run on your seasonal data
...
>important for forecasting or feeding a model to your dataset; for stochastic NOT monotonic
...
#defining the first month and year of your time series first will help when setting up your time series function
f_month <- month(first(wind_data_clean$DATE)) #first month will be the first observation of this column
f_year <- year(first(wind_data_clean$DATE)) #first year will be the first observation in this column
...
plot(wind_data_decomp) #find that the data is seasonal!

# Run SMK test - can handle seasonal series
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts)

# Inspect results
wind_data_trend1
summary(wind_data_trend1) #results - pvalue = 1.77e-11 --> reject null hypothesis that the data is stationary --> RESULT: since p-value < 0.05, reject Ho and find that the series has a trend
...
#can see value of S --> some seasons have a stronger tendency of decrease (higher negative value) and others are more stationary (number is closer to 0)
#p-value of seasons with stationary trend, have a p-value > 0.05 - this supports the null hypothesis!
...
> Answer: Over time, there is a decreasing wind speed over time
