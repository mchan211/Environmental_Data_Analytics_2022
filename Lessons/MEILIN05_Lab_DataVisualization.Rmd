...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #geom_text adds text to your plot, expression funciton allows you to add greek letters (ie: mu = mean) - closely related to how you enter an equation - R^2 is squared, R_2 is subscript (can find expression cheatsheets on google)
print(Plot_text)
...
# 1. 
# Plot surface temperatures by day of  year. need to filter for surface depth
# Color your points by year, and facet by lake in two rows.
# Change the ylab name - y-axis label
pp_surface_depth <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0.00)
...
surface_temp_dy <- ggplot(pp_surface_depth) +
  geom_point(aes(x=daynum, y= temperature_C, color = year4)) +
  facet_wrap(vars(lakename), nrow = 2) +
  ylab("Surface Temperature")


print(surface_temp_dy)
  


#2. 
# Plot temperature by date. Color your points by depth. geompoint
...
pp_temp_date <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C)) +
...
# Make the points 50 % transparent - use alpha argument in geompoint
# Add a line of best fit for the linear regression of these variables.

lat_o3_pm25 <- ggplot(EPAair, aes(x = PM2.5, y = Ozone))+
...
#scatter/dot plot - not good!
dotpm25_year <- ggplot(EPAair.year)+
  geom_point(aes(x= month, y = PM2.5, color = year))
print(dotpm25_year)

boxpm25_year <- ggplot(EPAair.year, aes(x = as.factor(year), y = PM2.5)) +
  geom_boxplot()
print(boxpm25_year)

barpm25_year <- ggplot(EPAair.year) +
  geom_bar(aes(x = year, y = PM2.5))
print(barpm25_year)
