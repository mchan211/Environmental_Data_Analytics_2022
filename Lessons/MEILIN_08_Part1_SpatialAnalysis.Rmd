...
```{r Read in road data}
#Read a spatial dataset - brings in a dataset from a webpage
GasPlants <- st_read('https://opendata.arcgis.com/datasets/a0722062315143a7980cebb79a204781_4.geojson') #THIS DID NOT WORK!!
...
class(GasPlants)
names(GasPlants)
str(GasPlants$geometry) #simple feature column_point (sfc_point)

#View using sf plot
plot(GasPlants['STATUS'],pch=16) #Status - either active or inactive

#View using ggplot
ggplot(data=GasPlants) + geom_sf() #geom_sf part of new sf package - shows in geographic space (latitude, longitude)

#View using mapview
mapview(GasPlants) #adds a map backdrop to your data
#pulls in the attributes - can see each point's attribute by clicking on it
#can play around with backdrops through mapview; can then export as a webpage
...
EPAair <- read.csv("./Data/Processed/EPAair_O3_PM25_NC2021_Processed.csv", stringsAsFactors = TRUE) %>% 
  mutate(Date = ymd(Date)) #this data has a lot of data for each site zone
...
#forging a geometry column in data frame through longitude and latitude --> use st_as_sf() --> creating st as a simple feature 

#Convert to a spatial dataframe
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'),
           crs=4326) #longitude is X, latitude is Y

#only has 8 variables - lost latitude and longitude (got collapsed within data frame)

#View the data using the methods above...
class(EPAair.sf) #has sf --> can do spatial analysis with it. 

plot(EPAair.sf['STATUS']) #DOES NOT WORK
...
> all use latitude and longitude in the next three dataframe types

```{r Create a spatial dataframe from scratch}

#simple feature geometry --> just location to a simple feature column (location + coordinate reference system) and data frame

#First create a simple feature geometry object ("sfg")

sfg_GraingerHall <- st_point(c(-78.9427,36.00465))
class(sfg_GraingerHall) #a point
mapview(sfg_GraingerHall) #doesn't know where that coordinate is in relation to a map

#Upgrade to a simple feature geometry column by adding a crs - coordinate reference system
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #crs refers to lat and long
#4326 is the log number of a coordinate referenc system by epsg.io (global geographic coordinate system catalogue) - "a look up number"

class(sfc_GraingerHall) #simple feature column
mapview(sfc_GraingerHall) #coordinate is now tied to a location on earth through the coordinate reference system

#Upgrade to a spatial dataframe - adds attributes
sf_GraingerHall <- st_as_sf(sfc_GraingerHall)
sf_GraingerHall$Name = 'Grainger Hall' #add a name attribute to the point
...
> downside is --> calculate distances in degrees (spherical coordinates)
>latitude and longitude are good for looking at a point on the earth, but not good for calculating distances - need planar coordinates by using PROJECTION
...
#View the result
mapview(the_buffer) #mapview did not show up for me?

#need to reconfigure data set so to lie on a plane --> projecting data is process of taking data on a sphere and mapping it to a flat or planar surface
#projection has challenges:
# 1.can have distortions since you are taking something spherical and trying to make it into a flat space
# 2. the earth is not perfectly spherical so there will be distortion when we project


#coordinate references - will project a multitude of earth models with different surface areas
```
...
> like putting graph paper of earth - determine distance by x and y units
>want to link systems - can get data sets to interact and "talk" with each other
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #COORDINATE POINTS ARE IN METERS --> the distance from the coordinate reference point; further you get from the point, more distortion

GasPlants.utm <- st_transform(GasPlants,32617)

#Have a look at the coordinate associated with the geometries
EPAair.sf.utm$geometry[1] #can see the units of the coordinates by looking it up on the database

#Plot the data - below is plotting more than one plot in one code
ggplot() +
  geom_sf(data = EPAair.sf.utm,color='blue') + 
  geom_sf(data = GasPlants.utm,color='red',shape='O') #lines have a bend to it because of distortion

#Create a distance matrix between the point in each dataset
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) #13 EPA sites, 32 gas plants sites

#Compute the mean of each row (each EPA site)
mean_distances <-  rowMeans(distance_matrix) #reduces the above data set to just row means --> collapsed all columns into a single column
#rowMeans() --> matrix command

#Add as a column to the epa sites
EPAair.sf.utm  <-  EPAair.sf.utm %>% 
  bind_cols(mean_distances) #bind the means to the original dataframe EPAair.sf.utm
