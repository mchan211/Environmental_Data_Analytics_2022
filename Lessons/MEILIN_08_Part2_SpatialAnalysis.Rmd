...
epa_pm25_sites_sf <- read_csv('./Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269)#collapsing by site just to get the mean and max PM for each site; then pipe into st_as_sf() to make into spacial feature and specify the x and y values with the EPSG number

#Inspect the object
class(epa_pm25_sites_sf)

#What is its CRS again?
st_crs(epa_pm25_sites_sf)$epsg #dropping $epsg will give entire description of the geographic coordinate system data source - "degree" = spherical coordinates/geographical
...
counties_sf<- st_read('./Data/Spatial/cb_2018_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties - STATEFP == 37 pulls just NC

#polygon --> geometry column is list of coordinate pairs that draw the shape of durham county
...
st_bbox(counties_sf) #coordinates of upper left and upper right box --> the spatial extents of the object

#View the data
head(counties_sf)

#Plot the data, colored by area of land in each county
mapView(counties_sf, zcol = "ALAND") #polygon dataset, zcol colors in an attribute (ie: AWATER, ALAND, etc.)
```
...
>HUCS = Hydrological Unit Code; ie: zipcode (more digits added, more specific it gets) - finest HUCS = 14 digits (very small spatial unit)
...
ggplot(data = huc8_sf) + 
  ...
  scale_fill_continuous(type = 'viridis') #to fill in colors, must use aes(fill = attribute) since it is a polygon (color=attribute will only color the outlines); use scale_fill_continous(type = 'color theme') to control the gradient of color
...
subhuc8_sf <- st_read("Data/Spatial/NCHUC8.shp") %>%
  filter(SUBBASIN == "Upper Neuse")

#Create a map
mapview(huc8_sf, col.regions = "orange") + mapview(subhuc8_sf, zcol = "SUBBASIN", col.regions = "purple") #?????

#putting color within aes() is for applying a color to specific feature values; if you just want a color for every attribute, put command outside of aes()
ggplot() +
  geom_sf(data = huc8_sf, fill = 'orange') +
  geom_sf(data = subhuc8_sf, fill = 'purple')
