...
HUC_wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor'))
...
huc_wind.8 <- left_join(HUC_wind, huc8_sf)

#alternative
#huc_wind.8 <- merge(x = huc8_sf, y = HUC_wind, by.x = 'HUC_8', by.y = 'HUC_8') - especialliy if the column names across data sets are different, need to specify which ones

#View the outputs
colnames(huc_wind.8)
summary(huc_wind.8)

ggplot(data = huc_wind.8) +
  geom_sf()
...
#Grab the intersecting HUC_8s - use of st_intersects()



#Intersect the HUC_8s - use of st_intersection() which works well with piping
