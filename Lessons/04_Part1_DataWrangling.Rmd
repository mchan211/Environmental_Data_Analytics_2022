...
`&`
>only one or the other to be true
`|`

```{r}
#always want to filter using "like to like" - makes sure the data is the same class
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# matrix filtering - this is filtering only the rows that have the value 0 in column depth
...
NTL.phys.data.surface2
NTL.phys.data.surface3
...
# Choose multiple conditions to filter - applies to rows FOR CHARACTER VARIABLE
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #lake can be peter OR paul lake

#below good for removing 1 or a couple names from a dataset
...
#after %in% type a vector with all the names you are looking for instead of having to type out lakenam == multiple times
...
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #daynum will be greater than 105 and less than 305
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #the comma is interpreted as using &
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #can put number range within a vecot using : to signifiy to pull values within this list
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #this last ratio for column names is the range of other columns that you are pulling to be shown select(df, column name, other columns)
NTL.phys.data.temps
...
```{r}
#pipe is like a chain of operations to use you must use '%>%'
NTL.phys.data.processed <- 
  NTL.phys.data %>% #only need to specify the name of your dataframe once in a pipe
