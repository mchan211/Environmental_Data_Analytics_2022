...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #the dataframe
  mutate(month = month(sampledate)) %>% #adding a column named month with the month of the sample date
  select(lakeid:daynum, month, sampledate:comments) %>% #want these specific columns
  drop_na(depth) #removing nas in depth column
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #transforms columns into rows (makes data longer)

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #subset() similar to filter() - inform OG data and checking if there is na in each row of column concentration
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) #this takes data and puts into columns
