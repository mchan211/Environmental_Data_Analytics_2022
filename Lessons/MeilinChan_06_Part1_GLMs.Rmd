...
---
```{r}
#^ above is asking outputs to be put in the console with plots being shown at right corner of the screen
```
>Part 1 focuses on CONTINUOUS VARIABLES
...
```{r}
#GLM = framework for compaing how vairables affect different continuous variable
```
...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #linear model function - provide your y-factor and then the independent variable

# another way to format the lm() function
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth) #irradiance water by depth (irradiance = y, depth = x)
summary(irradiance.regression)

#can see that p-value is less than t-value --> statistically different, can estimate irradiance in lakes based on depth
...
>  The test finds that there is a strong negative correlation between irradiance and depth of the lake. So, the deeper you go in a lake, the less irradiance there will be. 
...
plot(irradiance.regression) #linear model object
par(mfrow = c(1,1))
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000)#changing the plotting window +
  geom_point() 
print(irradiancebydepth) 

#can see that there is no linear relationship - can log transform the irradiance to depth (take log of irradiance values)
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #eliminating zeros from data set so you can take the log
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #taking the log of irradiancewater
...
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen) #2 independent variables
summary(TPregression)

#lm(data frame, explanatory variable ~ dependent variables-use + to add multiple dependent variables); if using subset within the function, then make sure you do lm(data = subset())
...
>correlation among any variables within your dataframe (visualization of which variables to add into your model)
...
corrplot(PeterPaulCorr, method = "ellipse") #thinner circle = higher correlation; color = positive or negative correlation (red = strong negative, dark blue = strong positive)
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #value of correlation w/ elipses

#corrplot shows candidates of variables to have in your regression based on AIC values
```
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #mulitple variable (anything after ~ is an explanatory variable)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #has R do one regression at a time
#analyzing the results - shows the removal of each variable and what the AIC values are - finds the variable combo that will allow you to reach the regression of best fit
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel) #3 optimal explanatory variables
