...
---
>Part 2 focuses on CATEGORICAL VARIABLES - interpret coefficents as constants to add to your response
...
#shapiro.test(column[row == site location])
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) #p-value < 0.05 - reject null hypothesis = data does NOT follow normal distribution
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_041"])
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) #follows normal distribution
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #follows normal distribution
...
#results: reject null in all except two: NIWO_057 and NIWO_046 - usually not enough
...
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID) #bartlett.test(variable ~ level)

#results: reject null i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova) #aov(data =  dataframe, continuous dependent variable ~ categorical variable) - similar to lm()


#results: reject null hypothesis i.e. difference between a pair of group means is statiscally significant
#sum square is deviation from each of observation from the mean 
#mean square = sum square/df (degrees of freedom)
#F value = mean square plot ID/mean square of residuals --> gives us P-value

# Format ANOVA as lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
#more detailed than aov() - difference between lm() and aov() is summary table
#summary of aov() = traditional anova output
#summary of lm() will have one row for each level of categorical variable
...
TukeyHSD(Litter.Totals.anova) #main argument should be the result of an anova test - aka using aov()
#shows: different of the mean, lower and upper end points of interval, p-value 

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #finds groups across different levels that have a similar mean #HSD.test(anova object, category, group = TRUE) group = TRUE specifies to group levels with same/similar mean
Litter.Totals.groups

# Graph the results
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+ #angles test since siteIDs are long 
...
> Can see variation in means. Would need to reorder the site to follow the order in the groups list - to see changes in level of mean visually
...
summary(Litter.anova.2way) #rejecting null hypothesis - mean across groups is different
...
```{r}
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass) # * means you want to see the interaction between the functionalGroup and nlcdClass
...
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass)) #has 24 levels - combo of functional group and class (interaction levels)
