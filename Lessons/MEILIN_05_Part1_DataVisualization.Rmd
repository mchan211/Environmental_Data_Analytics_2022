...
* geom_point
* geom_errorbar
* geom_smooth #add trendline to dataset
* geom_line
* geom_area
* geom_abline - point a to point b (plus geom_hline and geom_vline) #hline = horizontal, vline = vertical
* geom_text #add text to plot
...
# ^for the dimensions of any plot in our rcode chunk
...
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) #assinging a plot to a label is useful - plot objects can then be printed into same window oriented how you would like
print(O3plot)

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") #specifying color inside the aesthetic, you are adding in a new layer

#wrong way - geom_point(aes(x = Date, y = Ozone, color = "blue))
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? 
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + #asking R to automatically asign a specific shape to each point based on the year of the data collected
  geom_point()
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #creating separate plots for each site, and organizing the graphs into 3 rows
print(PMplot.faceted)

# Filter dataset within plot building and facet by multiple variables
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" | #can use subset within ggplot to select specific columns to graph
                  Site.Name == "Bryson City"), 
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #this is asking for each graph to be site name by year (columns = year, rows = sites selected)
...
lightvsDOfixed <- 
  ...
  geom_point() +
  xlim(0, 250) +
  ylim(0, 20) #setting axes limits to better visualize data
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #flips y-axis to have 0 start at the top
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #this adds a layer - method = lm adds a linear model (lm) = trendline
  geom_abline(aes(slope = 16, intercept = 0)) #need to specify the slope and intercept of line as aes()
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - color is the contour of the shape, fill will only change the inner coloring of the boxplot
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #binaxis = y is stacking the observations over y
#binwidth sets the width of bins
#stackdir = center --> how points are stacked (over center of boxes)
#postions = dodge --> won't overlay of dots for same concentrations (can see the actual density of data that is at that concentration for nutrients at lake)
print(Nutrientplot4)
...
#problems - no legends given so don't know which nutrient is which color --> not a good or easy frequency plot 
print(Nutrientplot6)

# Using a gathered dataset
Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient)) #much better and cleaner than using freqpoly for more than one variable
print(Nutrientplot7)
