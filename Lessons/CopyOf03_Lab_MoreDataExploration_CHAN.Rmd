...
#Before starting any project, upload from upstream on Terminal (git pull upstream main)
...
> Answer: vectors are listed under values, matrices are under lists, and data frames are under dataframe

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: You can pull up a specific 
...
```{r}
#2 vectors with 10 values each

hightempf <- c(39,41,43,34,42,50,48,62,65,56)
lowtempf <- c(19,28,31,16,27,25,31,46,48,27)
...
hightempc <- temp_conversion(hightempf)
lowtempc <- temp_conversion(lowtempf)
```
...
durham_weather <- data.frame("High in F"=hightempf,"High in C"=hightempc, "Low in F"=lowtempf, "Low in C"=lowtempc)
...
summary(durham_weather) #summary of whole data frame (min, median, mean, max and quartiles of each column)

summary(durham_weather$High.in.F) #shows the min, median, mean, max, and quartiles of this specific column

sd(durham_weather$Low.in.F) #cannot use on a whole data frame - must specify a column to calculate the standard dev.

sd(durham_weather$High.in.C)
sd(durham_weather$High.in.F)
sd(durham_weather$Low.in.C)
...
date1 <- format(today, format = "%m/%d/%y")
date2 <- format(today, format = "%B %d, %Y")
date3 <- format(today, format = "%A %B %d, %Y")

#make sure when you uploads dates from data, you transform object into numerical (R recognizes it usually as a character)
...
class(str_today)
#Since the format is year-month-day we will use function ymd() - package function when called will sense the year month date format
...
#there are other similar functions ydm(), mdy(), etc - change the used function based on the format the imported date is set in (ie: year month date, or date month year, etc.)
```
...
str_past <- "55-feb-3"
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=21L) #you have to specify the format the original date is in when you run the function #specifying cutoff - any years above 21 you add 19 to front of year, any years below 21 you add 20 to front of year
date_obj_past
