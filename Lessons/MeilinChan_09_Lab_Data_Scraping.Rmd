...
gage_data <- read.table(theURL,skip = 28,header=TRUE,sep='\t') #skip - telling R to skip this amount of first few rows - in this case, the chunk skipped is just the header on the webpage
...
newURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data <- read.table(newURL,skip = 28,header=TRUE,sep='\t') #skip - telling R to skip this amount of first few rows - in this case, the chunk skipped is just the header on the webpage
...
gage_data <- gage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime)) #ymd_hm() --> lubridate; year, month, day, hour, minute
...
pcode = '00060' #discharge (cfs) - parameter code

#Identify statistic code for daily values: 
scode = "00003"  #mean - stat mean

#Identify start and end dates - Date Window --> want to scrape data from then
start.date = "1930-10-01"
end.date = "2021-03-23"

#Load in data using the package's "readNWISdv" function - part of data retrieval package
...
neuse <- renameNWISColumns(neuse); #package automatically specifies package name
colnames(neuse)
...
```{r}
#Apply the API key - automatically stores your key within your R project
census_api_key("c5bb725f32cbaca831f441958559e7469624eff9", install = TRUE, overwrite = TRUE)
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE) #will automatically use your key to pull this data from census surveys
...
) #the values are all character fields - character strings with a comma cannot be put into as.numeric()
#gsub() will substitute a character - can use to take out the comma
...
the_data <- lapply(X = the_years, FUN = scrape.it) %>% bind_rows() #X = input, FUN = function

the_data <-map(the_years)
