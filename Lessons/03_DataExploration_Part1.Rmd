...
---
##git pull upstream main in terminal
...
```{r}
# Absolute file path is commented out - when you write out each folder by name = can set this when the working directory is not what you want it to be
...
read.csv("C:/Users/meili/OneDrive - Duke University/EDA/Environmental_Data_Analytics_2022/Data/Raw/USGS_Site02085000_Flow_Raw.csv")

# Relative file path (friendly for users regardless of machine) - use this when collaborating with a project, will work on multiple machines (significance of the period . --> "." means staying in current directory, ".." means moving to the parent folder)
USGS.flow.data <- read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv",stringsAsFactors = TRUE)

#"./Data/Raw/USGS_Site02085000_Flow_Raw.csv"  
...
str(USGS.flow.data)
dim(USGS.flow.data) #the number of rows and columns
length(USGS.flow.data) #the number of columns

head(USGS.flow.data)
tail(USGS.flow.data)
head(USGS.flow.data, 10) #shows the first 10 rows
tail(USGS.flow.data, 5) #shows last 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #shows the selected rows with columns 3, 8, and 14
...
> Answer: Wouldn't always remove for a time series analyis

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #na.omit will help omit the na from the data; create a new object to store the changed data
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #can see that rows have changed from original dataset as NAs have been removed
...
%b  abbreviated month    >abbreviated month (Jan)
%B  unabbreviated month  > not abbreviated month (January)
...
USGS.flow.data$datetime[1:10] #check the format of how the datetime is written in the original data

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #doing this for the original data set
...
create.early.dates <- (function(d) {
       ...
       }) #to fix any of the dates where the year is greater than 1999
#
