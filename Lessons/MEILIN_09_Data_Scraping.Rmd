...
> Make sure the website does not have a term of service for data. Some data has robot.txt which prevents scraping of data by bots --> can see if website has this by putting "/robots.txt" at the end of the url.
...
#going to teach computer to pull a few aspects from the data; and learn to pull vector of data (specific column)
webpage <- read_html('https://www.ncwater.org/WUDC/app/WWATR/report/view/0004-0001/2020') #pulls webpage into R Environment
webpage #set of nodes and set of documents
...
> go to webpage (in Chrome), right click and select "View Page Source"
...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>%  #copy pasted from Chrome using the SelectorGadget
  html_text()
#html_nodes() tells R to look at the webpage and within look for the nodes that fit the specific input -- use SelectorGadget to find the item (tag and value pair)

#html_text() --> tells R to give the value/text associated with node based on the tag and value pair
...
avg_withdrawals <- webpage %>% 
  ...
  html_text()
avg_withdrawals #character vector - R always pulls numeric values from websites as a character

max_withdrawals <- webpage %>% 
  html_nodes('.table:nth-child(7) td:nth-child(8) , .table:nth-child(7) td:nth-child(4)') %>%
  html_text()
max_withdrawals #character vector
```
...
#use as.numeric to convert the character vectors of pulled data into numeric vectors
#rep() creates sequence of numbers - in this situation, used to create the month column not scraped from webpage
...
#setting the pulled variables to each column
#!! --> telling R to refer to the following as VARIABLES instead of strings
